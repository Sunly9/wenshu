...
**核心发现：Rerank 不提升召回率（HitRate 持平 97.7%），但把 MRR 从 0.765 拉到 0.942——命中的块几乎全部被顶到第 1 名。交叉编码器的价值在排序质量，而非找得更多。**

生成质量（完整链路 + LLM 裁判，43 题）：**忠实度 100%**（回答全部有原文依据、零误拒），引用准确率 71.9%（115/160 角标真正支撑结论）。拒答阈值校准发现"相关性≠可答性"：无答案题的精排最高分（0.986）与可答题（最低 0.762）重叠——教材大篇幅讨论快速排序但不含其发明者，交叉编码器仍打高分。简单阈值无法完美分类，本系统取 0.35 零误拒优先，详见技术定稿 §8。完整指标口径与逐题型数据见调试台"评测面板"。
